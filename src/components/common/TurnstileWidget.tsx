...
/** 从 SCRIPT_SRC 解析出声明的 origin，用于运行时自校验 */
const DECLARED_SCRIPT_ORIGIN = (() => {
  try {
...
/** 校验待加载的 URL 是否与声明的脚本同源，防止常量被内存篡改后加载恶意脚本 */
function isAllowedScriptUrl(url: string): boolean {
  try {
    const parsed = new URL(url);
    return parsed.origin === DECLARED_SCRIPT_ORIGIN && DECLARED_SCRIPT_ORIGIN !== '';
  } catch {
    return false;
  }
}
...
function loadTurnstileScript(): Promise<void> {
  ...
  // 安全校验：确保脚本来源与声明地址一致，防止内存篡改常量后加载恶意脚本
  if (!isAllowedScriptUrl(SCRIPT_SRC)) {
    return Promise.reject(new Error('Turnstile script source does not match declared origin'));
  }
